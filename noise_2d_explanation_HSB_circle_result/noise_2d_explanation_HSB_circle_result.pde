...
void draw()
{
 ...
 //hue
  ...
  // }
  for (int i = 0; i < LEDCount; ++i) {
    float noiseValH = noise(xCoords[i] * step, yCoords[i] * step + 50, zIndex);
    float noiseValS = noise(xCoords[i] * step, yCoords[i] * step + 50, zIndex);
    float noiseValB = noise(xCoords[i] * step, yCoords[i] * step, zIndex);

    float hue = (colorCenter - (colorRange / 2) + noiseValH * colorRange + 360) % 360;
    color c = color(hue, noiseValS*100, noiseValB*50+50);
    fill(c);    
    rect(xCoords[i] * boxSize + leftMargin2, yCoords[i] * boxSize + topMargin1, boxSize, boxSize);
  }
  // //saturation
  //for (int row = 0; row < diameter; ++row)
  //{
  ...
  //}
  for (int i = 0; i < LEDCount; ++i) {
    float noiseVal = noise(xCoords[i] * step, yCoords[i] * step + 100, zIndex);
    color c = color(colorCenter, 100 * noiseVal, 100);
    fill(c);    
    rect(xCoords[i] * boxSize + leftMargin3, yCoords[i] * boxSize + topMargin1, boxSize, boxSize);
  }
